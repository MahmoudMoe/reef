...
## Layout
skills/ (init, plan, plan-review, task, verify, review) · agents/ (implementer, verifier, plan-reviewer — each carries a DEFAULT model; the dispatch-time model from reef-attempt overrides it) · scripts/ (reef-gate.sh, reef-attempt, reef-guard.py, reef-snapshot.sh, reef-plan-check.py) · schemas/task.schema.json · templates/ · tests/ (sabotage suites: every gate shown red)
