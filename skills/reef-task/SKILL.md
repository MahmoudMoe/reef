...
Run `scripts/reef-attempt <task-file>`. Exit 2 = blocked -> print its USER ACTION REQUIRED and STOP the pipeline. Otherwise use EXACTLY what it prints: pass its model as the Agent call's model parameter (overrides the frontmatter default), state its effort in the dispatch prompt (e.g. "reasoning effort: high") — agent-frontmatter effort keys are inert — and START THE DISPATCH PROMPT with the `REEF-TASK: <task-file>` line it printed. That line is not decoration: the plugin's PreToolUse guard refuses any implementer dispatch without it, refuses blocked/at-cap tasks, re-verifies the plan stamp, and advances the task's `dispatches:` odometer — the cap holds even if you forget this step.
...
4. `status: blocked` → USER ACTION REQUIRED, stop. Never reset attempts yourself — that is a human edit. The full human unblock is THREE fields: `attempts: 0`, `status: pending`, `last_failure_sig: ""` (a stale signature would block the fresh round on its first failure).
