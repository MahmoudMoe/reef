...
- Issues found → bundle into ONE rollup task file (`tasks/R<NN>-<slug>.md`, template frontmatter, `complexity` judged fresh) → hand to `/reef-task` → re-run this step. Use `scripts/reef-attempt` on the rollup for the cap: 3 acceptance rounds max, then USER ACTION REQUIRED.
...
`gh run watch` (or `gh pr checks --watch`). Red CI → read the actual failing log (never guess from the test name), write the failure into a rollup task, `/reef-task` it, push, re-watch. 5 rounds max.
